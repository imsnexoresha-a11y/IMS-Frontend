...
export async function updateInstructorProfile(userId, updateData, profileImagePath) {
  ...
  const { name, designation, bio, linkedInUrl } = updateData;
...
export async function getInstructorBatches(userId) {
  ...
  const [studentBatches, sessionBatches] = await Promise.all([
    Student.find({ enrolledCourseIds: { $in: courseIds } }).distinct('batchId'),
    Session.find({ courseId: { $in: courseIds } }).distinct('batchId'),
  ]);

  const allBatchIds = [...new Set([...studentBatches, ...sessionBatches])].filter(Boolean);
  const batches = await Batch.find({ _id: { $in: allBatchIds } });

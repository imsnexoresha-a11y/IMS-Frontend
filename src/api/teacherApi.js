...
export async function createSessionAssignment(sessionId, data) {
  if (!USE_MOCK) {
    return apiClient.post(`/instructor/sessions/${sessionId}/assignment`, data);
  }
  const lec = mockLectures.find((l) => l.id === sessionId || l._id === sessionId);
  if (lec) {
    lec.assignmentTitle = data.assignmentTitle;
    lec.assignmentDescription = data.assignmentDescription;
    lec.assignmentDeadline = data.assignmentDeadline;
    lec.githubRepoSeed = data.githubRepoSeed;
  }
  return mockResponse({ success: true, message: 'Assignment created successfully (Mock)' });
}
